#!/usr/bin/env python3
"""Fixed version of simple_generate.py with proper path handling.

This wrapper script calls ACE-Step with correct Python path setup.
...
# ============================================================================
# FIX #1: Correct Python path setup
# ============================================================================
def setup_python_path():
    """Setup correct Python paths for ACE-Step imports."""
    
    # Get the directory where this script is located
    script_dir = Path(__file__).resolve().parent
    
    # Navigate to repo root: server/scripts -> server -> ace-step-ui -> root
    repo_root = script_dir.parent.parent.parent
    acestep_dir = repo_root / 'ACE-Step-1.5_'
    
    # Verify structure
    if not acestep_dir.exists():
        raise EnvironmentError(
            f"ACE-Step-1.5_ not found at {acestep_dir}\n"
            f"Expected structure: repo_root/ACE-Step-1.5_/acestep/\n"
            f"Got: {script_dir}"
        )
    
    # Add repo root to path (so 'from acestep...' imports work)
    repo_root_str = str(repo_root)
    if repo_root_str not in sys.path:
        sys.path.insert(0, repo_root_str)
    
    # ========================================================================
    # FIX #2: Set environment variables properly
    # ========================================================================
    os.environ['ACESTEP_PATH'] = str(acestep_dir)
    os.environ['PYTHONPATH'] = f"{repo_root_str}{os.pathsep}{os.environ.get('PYTHONPATH', '')}"
...
# Setup paths BEFORE importing acestep modules
try:
...
def get_handlers():
    """Get or initialize ACE-Step handlers."""
...
def generate(
    ...
    if output_dir is None:
        output_dir = os.path.join(paths['acestep_dir'], "output")
...
def main():
    ...
    if args.diagnostics:
        ...
        print(f"  ACE-Step Dir: {paths['acestep_dir']}")
        print(f"  Venv Active: {paths['venv_active']}")

...
if exist "ACE-Step-1.5\.venv" (
    echo [OK] Python virtual environment found at ACE-Step-1.5\.venv
) else (
    echo [WARNING] Python virtual environment not found at ACE-Step-1.5\.venv
    echo [INFO] Run 'Install/Setup' from the main menu to create it
)

echo.
echo Checking checkpoints directory...
if exist "%USERPROFILE%\AI\ProdIA_Max-Ace-Step-UI_Ace-Step\checkpoints" (
    echo [OK] Checkpoints directory found
) else (
    echo [WARNING] Checkpoints directory not found
    echo [INFO] Expected: %USERPROFILE%\AI\ProdIA_Max-Ace-Step-UI_Ace-Step\checkpoints
)
...
if exist "ACE-Step-1.5\generated_audios" (
    echo Deleting generated audio folder...
    rmdir /s /q "ACE-Step-1.5\generated_audios" >nul 2>&1
)
...
if exist "ACE-Step-1.5\.venv" (
    rmdir /s /q "ACE-Step-1.5\.venv" >nul 2>&1
)
...
echo Creating virtual environment in ACE-Step-1.5\.venv...
cd "ACE-Step-1.5"
python -m venv .venv
if !errorlevel! neq 0 (
...
if !errorlevel! neq 0 (
    ...
    echo [INFO] Continuing with existing pip version...
)
...
start "ProdIA-MAX Gradio API" cmd /k "cd ACE-Step-1.5 && call .venv\Scripts\activate.bat && python -m acestep --port 8001 --server-name 127.0.0.1"
